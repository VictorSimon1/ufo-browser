# CLI parity with Ego 1.2.6

## Capability matrix

| Area | Ego Skill / CLI | X-Browser | Notes |
|---|---|---|---|
| Heredoc execution | `ego-browser nodejs` | `x-browser nodejs` | Same async global-helper execution model; local helper-name shadowing remains legal. |
| Task spaces | list, create/reuse, claim, handoff, takeover, complete | Aligned | X-Browser additionally enforces a generation lease on every host mutation and CDP command. |
...
| Site skills | discovery, Node tools, browser tools, learned context | Aligned | Resources resolve from `skills/x-browser`, not from an Ego installation. |
| Help and facades | flat helpers plus page/browser/taskSpaces/site | Aligned | X-Browser keeps the complete flat compatibility surface and the structured facades. |

## Deliberate differences

- The executable and Skill name are `x-browser`; no `ego-lite` package or runtime is loaded.
- Stable `EGO_*` error codes remain accepted for script compatibility even though the implementation is owned by X-Browser.
- Profile/Cookie import is intentionally not implemented yet; normal Spaces share the X-Browser profile partition.
- X-Browser adds bounded Overview renderer, preview-cache, and GPU cadence controls outside the CLI helper contract.
