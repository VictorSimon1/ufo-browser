...
- `storage-preflight.ts` validates LevelDB manifests and SQLite quota metadata without reading stored values, discovers only bounded HTTP(S) origins needed for the runtime probe, and removes structurally invalid datasets before a target Session exists. `storage-preflight-worker.ts` runs that filesystem/SQLite work outside Electron's main event loop and exposes only a stable Worker failure code.
...
Before Keychain access, the activated copy receives a bounded static compatibility preflight in a Worker. Local Storage, IndexedDB, and File System LevelDB directories must have a consistent `CURRENT`/`MANIFEST` structure, while copied QuotaManager SQLite databases up to the bounded inspection size must pass `quick_check`; larger metadata files defer their format decision to Chromium instead of synchronously scanning on the UI thread. If this already proves that an origin-storage dataset is unsafe and partial import was not approved, the transaction stops before requesting the Chrome Safe Storage secret.
