...
export async function ensureIframeSession(frameId: string) {
  const existing = iframeSessions.get(frameId);
  ...
  if (!sessionId) {
    throw new Error(`Could not attach iframe target: ${frameId}`);
  }
  iframeSessions.set(frameId, sessionId);
  return sessionId;
}

export function browserIframeSessions() {
...
function handleMessage(message) {
  ...
  for (const subscriber of eventSubscribers) {
    ...
    try {
      ...
      console.error(`browser event subscriber failed for ${data.method}:`, error);
    }
  }
